...
public class SPORun {
    ...
    public static void main(String[] args) {
        ...
        double initialPR = 0.3;
        double sumPR = 0;

        DoubleChomosome[] doublePopluation = doublePopulationGenerator.generatePopulation(1000, new Deb4Function(2));
        ...
        for (int t = 0; t < numberOfRuns; t++) {
            ...
            if (neighbourSize1 < 0 || neighbourSize1 > 1) {
                neighbourSize1 = neighbourSize;
            }
            if ((eps1 < 0) || eps1 > 0.5) {
                eps1 = eps;
            }
            if (changespeed1 < 1 || changespeed1 > 10) {
            ...
            if (
                    //(Math.abs(statisticsNew.peakRatio-1)<Math.abs(statistics.peakRatio-1))
                 //   &&(statisticsNew.exactRatio>statistics.exactRatio)&&
                 (statisticsNew.exactRatio > initialPR) && (Math.abs(statisticsNew.peakRatio - statisticsNew.exactRatio) < 0.0001)) {
                success++;
            }
            ...
            System.out.println("N = " + populationSize1 + ", neighbour = " + neighbourSize1 + ", eps = " + eps1 + ", speed = " + changespeed1);
